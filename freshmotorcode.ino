...
void zigZag(){ //clear function
  ...
  float offset = 2;
  float step = 2;
  plot(x, y, width, height - offset);
  while(width - offset > 0){
    plot(x, y, width - offset, height);
    left(step);
    offset += step;
    plot(x, y, width, height - offset);
    down(step);
...
void growTriangles(float x2, float y2, float x3, float y3){ // arguments should be ACTUAL POINTS, within table range
  center();
  float x1 = x;
  float y1 = y;
  ...
  while(inRange(coordinates)){ // check if points are in range
    ...
    plot(x, y, x2, y2);
    plot(x, y, x3, y3);
    for(int i = 0; i < 6; i++){ // add to all coordinates, growing shape
      coordinates[i] += 2;
    }
  }
}
...
bool inRange(float coordinates[]){ // check if coordinates are in range, even=x, odd=y
  ...
  for(int i = 0; i < size; i++){ //check if inRange
    if (fmod(coordinates[i], 2) == 0){ // if x coordinate
      if(coordinates[i] >= width || coordinates[i] <= 0){
        return false;
      }
    }
    if (fmod(coordinates[i], 2) != 0){ // if y coordinate
      if(coordinates[i] >= height || coordinates[i] <= 0){
        return false;
      }
    }
  }
